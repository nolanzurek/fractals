...
void draw() {
  ...
  for(int i = 0; i < pixels.length; i++) {
    ComplexNumber C = new ComplexNumber(map(i%width, 0, width, -2, 2), map(i/height, 0, height, -2, 2));
    pixels[i] = color(map((float)(applyJulia(C, 20)), 0, 20, 0, 255));
...
int applyJulia (ComplexNumber C, int depth) {
  ComplexNumber acc = new ComplexNumber(0, 0);
